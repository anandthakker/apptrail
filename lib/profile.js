...
module.exports = function(route, elevation, shelters) {
  ...
  var y = scale([0, maxElevation], [100,0])
  var x = scale(extremes(elevation.map(function(d) {return d.trailDistance})), [0,400])

  var pointString = elevation
      .map(function(d) { return x(d.trailDistance) + ',' + y(d.ele) })
      .join(' ')

  return h('svg', { viewBox: '0 0 400 100', preserveAspectRatio: 'none' }, [
    h('polyline', {
      class: 'trail',
      fill: 'none',
      points: pointString
